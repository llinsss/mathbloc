...
import { type LeaderboardEntry } from '@/lib/useContract';

export default function Web3Panel() {
  const { connect, register, claimReward, getLeaderboard, player, address, loading, error, connected, isDeployed, contractAddress, correctNetwork, switchNetwork } = useWeb3();
      ...
      {showPanel && (
        <div className="card-game border-indigo-200 p-4 space-y-4 animate-bounce-in">
          <div className="bg-indigo-50 rounded-xl p-3 border border-indigo-200">
            <p className="text-xs font-black text-indigo-500 uppercase">Contract (Celo)</p>
            ...
            <div className="space-y-3">
              ...
              {!correctNetwork && (
                <button
                  ...
                  className="btn-game w-full bg-gradient-to-b from-orange-400 to-orange-500 border-4 border-orange-600 py-3 text-sm"
                >
                  ⚠️ Switch to Celo Network
                </button>
              )}

              {!player?.exists && profile && correctNetwork && (
                <button
